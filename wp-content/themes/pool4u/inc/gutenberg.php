...
function hs_acf_blocks_init() {
	...
	if ( function_exists( "acf_register_block_type" ) ) {
		acf_register_block( array(
			"name"				=> "projects",
			"title"				=> __( "Проекты", "pool4u" ),
			"description"		=> __( "Показать все проекты", "pool4u" ),
			"render_callback"	=> "hs_acf_render_block",
			"category"			=> "layout",
			"keywords"			=> array( "projects","project","проекты" ),
		...
		acf_register_block_type( array(
			"name"				=> "contact-form",
			"title"				=> __( "Форма контактов", "pool4u" ),
			"description"		=> __( "Вставляет форму для контактов на страницу", "pool4u" ),
			"render_callback"	=> "hs_acf_render_block",
			"category"			=> "design",
			"keywords"			=> array( "схема","форма", "контакт", "form", "contact" ),
			"icon"				=> "email-alt",
			"supports"			=> array(
				"align"				=> array("wide"),
			),
